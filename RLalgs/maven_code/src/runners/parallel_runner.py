...
class ParallelRunner:
    ...
    def __init__(self, args, logger):
        ...
        self.ps = [Process(target=env_worker, args=(worker_conn, CloudpickleWrapper(partial(env_fn, env=None, **self.args.env_args))))
    ...
    def run(self, test_mode=False, test_uniform=False):
        ...
        while True:
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    episode_returns[idx]["reward"] += data["reward"][0]
                    episode_returns[idx]["delta_enemy"] += data["reward"][1]
                    episode_returns[idx]["delta_deaths"] += data["reward"][2]
                    episode_returns[idx]["delta_ally"] += data["reward"][3]
        ...
        for d in episode_returns:
            for k in d:
                cur_returns[k].append(d[k])


        self._update_noise_returns([d['reward'] for d in episode_returns], self.noise, final_env_infos, test_mode)
        self.noise_distrib.update_returns(self.batch['state'][:,0], self.noise, [d['reward'] for d in episode_returns], test_mode, self.t_env)
