...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_unit_type_id(self, unit, ally):
        ...
        if ally:  # use new SC2 unit types
            ...
            if self.map_type == "stalkers_and_zealots":
                ...
                # id(Hydralisk) = 107, id(Stalker) = 74, id(Zealot) = 73,
                if unit.unit_type == 107:
                    type_id = 0
                elif unit.unit_type == 74:
                    type_id = 1
    ...
    def update_units(self):
        ...
        if (
            ...
            or self.check_nexus()
            or self.check_enemy_unit_killed()
        ):
            return 1  # won
    ...
    def check_enemy_unit_killed(self):
        """Check if all the enemy's units are killed, except buildings"""
        if self.map_type != "sdjx":
            return False
        
        for e in self.enemies.values():
            if e.unit_type != 59 and e.unit_type != 133 and e.health > 0:
                return False
