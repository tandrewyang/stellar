...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        if self.map_type == "sdjx" and self.n_agents - map_params["n_madivac"] > self.n_enemies:
        ...
        # self.nexus_id = self.warpgate_id = 0
        self.roach_id = self.hatchery_id = self.nydusNetwork_id = self.nydusCanal_id = 0
        self.sentry_id = 0
    ...
    def _launch(self):
        ...
        # print self.pathing_grid and self.terrain_height
        # import matplotlib.pyplot as plt
        # plt.imshow(self.terrain_height, cmap='gray', alpha=0.5)
        # plt.title('Pathing Grid')
        # plt.show()
        # plt.imshow(self.pathing_grid, cmap='Reds', alpha=0.5)
        # plt.title('Terrain Height')
        # plt.show()
        # plt.imshow(self.terrain_height, cmap='gray', alpha=0.5)
        # plt.imshow(self.pathing_grid, cmap='Reds', alpha=0.5)
        # plt.title('Combined Pathing Grid and Terrain Height')
        # plt.show()
    ...
    def get_agent_action(self, a_id, action):
        ...
        if action == 0:
        ...
        elif self.map_type == "dhls" and a_id in self.load:
            return None
        ...
        elif self.map_type == "dhls" and action == 6 and unit.unit_type == self.nydusNetwork_id:
            # Load all units in the sight range
            target_tag = 0
            for t_id, t_unit in self.agents.items():
                if (t_unit.unit_type == self.roach_id and 
        ...
        elif self.map_type == "dhls" and action == 6 and unit.unit_type == self.nydusCanal_id:
            # Unload all agents
            # target_tag = []
            # for t_id, t_unit in self.load.items():
            #     target_tag.append(t_unit.tag)
            cmd = r_pb.ActionRawUnitCommand(
        ...
        elif self.map_type == "wzsy" and action == 6:
            ...
            if (self.map_type == "MMM" or self.map_type == "sdjx") and unit.unit_type == self.medivac_id:
    ...
    def reward_battle(self):
        ...
        # update deaths
        for al_id, al_unit in self.agents.items():
            if al_unit != None and not self.death_tracker_ally[al_id]:
                # did not die so far
                prev_health = 0
    ...
    def get_state_dict(self):
        ...
        for al_id, al_unit in self.agents.items():
            if al_unit != None and al_unit.health > 0:
                ...
                if (
                    (self.map_type == "MMM" or self.map_type == "sdjx")
    ...
    def get_unit_type_id(self, unit, ally):
        ...
        if ally:  # use new SC2 unit types
            ...
            if self.map_type == "wzsy" and unit.unit_type == 74:
                type_id = 2
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        if unit != None and unit.health > 0:
            ...
            if self.map_type == "dhls" and unit.unit_type != self.roach_id:
                # the structures in dhls can only stop
                avail_actions[1] = 1

                if unit.unit_type == self.nydusNetwork_id:
                # check if roach can enter the NydusNetwork
                    for t_id, t_unit in self.agents.items():
                        if t_unit.unit_type == self.roach_id and t_id not in self.load:
                            dist = self.distance(
                ...
                if unit.unit_type == self.nydusCanal_id and self.load != {}:
                    avail_actions[6] = 1
            ...
            if self.map_type == "wzsy":
                if (unit.unit_type == self.sentry_id and
                    unit.energy >= 100 and
    ...
    def _init_ally_unit_types(self, min_unit_type):
        ...
        if self.map_type == "marines":
            ...
            self.hatchery_id = min_unit_type
            self.nydusNetwork_id = min_unit_type + 1
            self.nydusCanal_id = min_unit_type + 2
            self.roach_id = min_unit_type + 3
        elif self.map_type == "wzsy":
    ...
    def only_medivac_left(self, ally):
        ...
        if self.map_type != "MMM" and self.map_type != "sdjx":
            return False
    ...
    def check_structure(self, ally = True):
        ...
        if self.map_type == "dhls":
            if ally == True:
                for a in self.agents.values():
                    if a.unit_type == self.hatchery_id and a.health <= 0:
                        return True
    ...
    def check_unit_killed(self, ally = True):
        ...
        if self.map_type == "dhls":
            ...
            if ally == True:
                for a in self.agents.values():
                    if a.unit_type == self.roach_id and a.health > 0:
                        return False
                return True
    ...
    def clean_load(self):
        ...
        for a_id, a_unit in self.agents.items():
            if a_unit.unit_type == self.nydusCanal_id:
                del_id = []
