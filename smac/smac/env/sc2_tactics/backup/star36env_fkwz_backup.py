    ...
    def get_agent_action(self, a_id, action):
        ...
        if action == 0:
            ...
            if self.resource >= 100:
                for t_unit in self.agents.values():
                    if t_unit != None and t_unit.unit_type == self.rlunit_ids.get("pylon"):
                        if target_id == 0:
                            self.update_offset()
                            cmd = r_pb.ActionRawUnitCommand(
                                ability_id=actions["warpgateTrain"],
                                target_world_space_pos=sc_common.Point2D(
                                    x=t_unit.pos.x + self.offset_x, y=t_unit.pos.y + self.offset_y
                                ),
                                unit_tags=[tag],
                                queue_command=False,
                            )
            ...
            if unit.unit_type == self.rlunit_ids.get("warpgate"):
                if self.resource >= 100 and self.cooldown_dict[agent_id] == 0:
                    ...
                    for a_unit in self.agents.values():
                        if a_unit != None and a_unit.unit_type == self.rlunit_ids.get("pylon"):
                            if a_unit.health > 0:
                                avail_actions[6 + pylon_idx] = 1
                            pylon_idx += 1
                return avail_actions
    ...
    def check_structure(self, ally = True):
        ...
        if ally == True:
            for a in self.agents.values():
                if a != None:
                    if (a.unit_type == self.rlunit_ids.get("pylon") or
                        a.unit_type == self.rlunit_ids.get("warpgate")):
                        if a.health <= 0:
                            return True
        return False
