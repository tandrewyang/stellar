...
class SC2TacticsWWJZEnv(te.SC2TacticsEnv):
    ...
    def get_agent_action(self, a_id, action):
        ...
        elif action == 6 and unit.unit_type == self.rlunit_ids.get("warpPrism_rl"):
            # Load all units in the sight range
            target_tag = 0
            for t_id, t_unit in self.agents.items():
                if ((t_unit.unit_type == self.rlunit_ids.get("stalker_rl") or
                    t_unit.unit_type == self.rlunit_ids.get("zealot_rl")) and 
        ...
        elif action == 7 and unit.unit_type == self.rlunit_ids.get("warpPrism_rl"):
            # Unload an agent
            cmd = r_pb.ActionRawUnitCommand(
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        if unit.health > 0:
            ...
            if unit.unit_type == self.rlunit_ids.get("nexus_rl"):
            ...
            if unit.unit_type == self.rlunit_ids.get("warpPrism_rl"):
                # check if WarpPrism can load units
                for t_id, t_unit in self.agents.items():
                    if (len(self.load) < self.get_load_max() and
                        (t_unit.unit_type == self.rlunit_ids.get("stalker_rl") or
                        t_unit.unit_type == self.rlunit_ids.get("zealot_rl")) and 
                        t_id not in self.load):
                        dist = self.distance(
    ...
    def check_structure(self, ally = True):
        ...
        if ally == True:
            for a in self.agents.values():
                if a.unit_type == self.rlunit_ids.get("nexus_rl") and a.health <= 0:
                    return True
    ...
    def clean_load(self):
        ...
        for a_id, a_unit in self.agents.items():
            if a_unit.unit_type == self.rlunit_ids.get("warpPrism_rl"):
